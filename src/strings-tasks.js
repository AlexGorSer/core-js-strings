...
function isPalindrome(str) {
  const lowerStr = str.toLowerCase().replace(/[^a-z]/gi, '');

  let leftChar = 0;
  let rightChar = lowerStr.length - 1;

  while (leftChar < rightChar) {
    if (lowerStr[leftChar] !== lowerStr[rightChar]) return false;
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < arrOfWords.length; i += 1) {
    if (arrOfWords[i].length > longWord.length) {
      longWord = arrOfWords[i];
    }
  }
  return longWord;
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((elem) => (input.includes(elem) ? output[input.indexOf(elem)] : elem))
    .join('');
}
